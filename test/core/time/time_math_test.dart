...
void main() {
  ...
  group('week ranges', () {
    ...
    test('rejects invalid visible day counts in release-safe code', () {
      expect(
        () => WeekRange(
  ...
  group('rounding', () {
    test(
      'rounds minutes since midnight to nearest 5 minutes with day offset',
      () {
        expect(
          roundMinutesSinceMidnightToNearestInterval(7),
          RoundedTimeOfDayMinutes(
            time: TimeOfDayMinutes.fromHourMinute(hour: 0, minute: 5),
            dayOffset: 0,
          ),
        );
        expect(
          ...
          RoundedTimeOfDayMinutes(
            time: TimeOfDayMinutes.fromHourMinute(hour: 0, minute: 10),
            dayOffset: 0,
          ),
        );
        expect(
          roundMinutesSinceMidnightToNearestInterval(1438),
          RoundedTimeOfDayMinutes(
    ...
    test('rounds UTC DateTime inputs from UTC midnight', () {
      final rounded = roundDateTimeToNearestInterval(
        DateTime.utc(2026, 7, 5, 9, 2, 30),
      );

      expect(rounded, DateMinute.fromDateTime(DateTime.utc(2026, 7, 5, 9, 5)));
    });
